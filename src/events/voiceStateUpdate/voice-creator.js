...
module.exports = async (oldState, newState) => {
    ...
    if (oldState.channel !== newState.channel && newState.channel && newState.channel.id === newState.channel.id) {
        ...
        // Check if the channel parent has "group chats" in the name.
        if (channel.parent.name.includes('group chats')) {
            // Check if the role "Gamer Access" exists in the guild - if it doesn't then we can stop here.
            const role = newState.guild.roles.cache.find(role => role.name.includes('Gamer Access'));
            if (!role) {
                return;
            }
